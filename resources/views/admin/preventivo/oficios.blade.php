...
<div class="contenido">
	...
	<div class="col-md-7">
		<form id="form" action="{{route('oficios.index')}}" method="get" class="mt-2 mb-5">
			<div class="row mb-4">
				<div class="col">
        ...
        <tbody>
			@foreach($oficios as $oficio)
				<tr>
					...
					<td>
						<p>{{$oficio->departamento->nombre}}</p>
					</td>
					<td>
						@foreach($fechas as $fecha)
							@if ($oficio->id == $fecha->cronograma_preventivo_id)
								<p>{{$fecha->fecha}}</p>
							@endif
						@endforeach	
					</td>
                    ...
                    <td>
						<a href="{{route('oficios.show', $oficio->id)}}" target="_blank">
	...
	<div class="mt-3">
		{{ $oficios->appends(request()->input())->links() }} 
	</div>	
</div>
@endsection
